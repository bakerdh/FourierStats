...
nsims <- 100000  # global value for the whole script - increase to 100000 for final run, but note this takes around 24 hours to complete (depending on processor speed)
buildfigs <- 1

# reasonably compact code to check which packages are installed, 
# install the missing ones, and activate all
packagelist <- c('bookdown','pals','pracma','shape','reshape2','diagram','devtools','knitr')
...
knitr::include_graphics('figures/fourierexplain.pdf')
...
pdf('figures/amphists.pdf', bg="transparent", height = 6, width = 12)
...
```{r amphists, fig.cap="Demonstration of skew in absolute Fourier amplitudes for signals of different strengths. Signal strength is quantified as Cohen's d, defined as the ratio of the sample mean to the sample standard deviation. The upper row shows samples of 50 grey points, and the population mean (coloured points). The lower row shows kernel density functions generated from 100,000 amplitude values. Note that the mean phase of the signal is irrelevant for these simulations, and is shown in the positive x-direction for consistency.", fig.align="center", echo=FALSE, fig.width=12, fig.height=6}

knitr::include_graphics('figures/amphists.pdf')

```

The upper row of Figure \@ref(fig:amphists) shows scatterplots of simulated Fourier components, expressed using real (_x_) and imaginary (_y_) components. The amplitudes are the lengths of the lines joining each grey point to the origin. The lower row in Figure \@ref(fig:amphists) shows distributions of amplitudes for the same set of signal strengths. These distributions only approach normality when the signal strength is more than twice the standard deviation (Cohen's _d_ > 2; Cohen's _d_ is the difference of sample means scaled by the standard deviation, see Cohen [-@Cohen1988]). One consequence of this is that T-tests will potentially have an inflated Type I error (false positive) rate for many signals encountered empirically, especially if used to make pointwise comparisons to an amplitude of 0.
...
# Comparison of statistical power between univariate and multivariate tests

Using the $T^2$ or $T^2_{circ}$ statistic allows the phase information to be retained, and therefore provides greater power than running univariate T-tests on amplitudes, as well as avoiding problems caused by using absolute amplitude values. Victor & Mast [-@Victor1991] report simulations showing situations where $T^2_{circ}$ has greater power than $T^2$, which is expected on theoretical grounds because the additional degrees of freedom in the $T^2_{circ}$ test make it more efficient. Their simulations involved generating random data sets of different sample sizes, and different signal strengths (sample means), and comparing the number of such tests where each statistic was significant. The simulations showed the largest advantage for $T^2_{circ}$ for effect sizes around _d_ = 1 (where the sample mean is equal to the sample standard deviation). The advantage appeared to be stronger for smaller sample sizes.
...
pdf('figures/powerfig.pdf', bg="transparent", height = 8, width = 12)
...
actualslog <- log2(samplesizes)


tmatrix <- matrix(0,nrow=length(samplesizes),ncol=length(dlist))
tmatrix2 <- matrix(0,nrow=length(samplesizes),ncol=length(dlist))
tmatrix3 <- matrix(0,nrow=length(samplesizes),ncol=length(dlist))
...
for (n in 1:nsims){
  for (s in 1:length(samplesizes)){
    for (s2 in 1:length(dlist)){
      baseline <- matrix(rnorm(2*samplesizes[s]),nrow=samplesizes[s],ncol=2)
      ...
      ampdata <- abs(complex(real = data[,1], imaginary = data[,2]))
      ampbase <- abs(complex(real = baseline[,1], imaginary = baseline[,2]))
      a <- t.test(ampbase,ampdata)
      if (a$p.value<0.05){tmatrix[s,s2] <- tmatrix[s,s2] + 1} 
      
      a <- t.test(data[,1])
      b <- t.test(data[,2])
      if (a$p.value<0.05 | b$p.value<0.05){tmatrix2[s,s2] <- tmatrix2[s,s2] + 1}
      if (a$p.value<0.025 | b$p.value<0.025){tmatrix3[s,s2] <- tmatrix3[s,s2] + 1}
      
    }}
}

tpropM <- tmatrix/nsims
tpropM2 <- tmatrix2/nsims
tpropM3 <- tmatrix3/nsims
...
for (n in 1:3){
  ...
  lines(actualslog,tpropM3[,vinds[n]],lwd=3,col='black',lty=2)
  lines(actualslog,tpropM2[,vinds[n]],lwd=3,col='grey')
  lines(actualslog,tpropM[,vinds[n]],lwd=3,col=nbdtpal[3])
  lines(actualslog,tsqpropM[,vinds[n]],lwd=3,col=nbdtpal[1])
  lines(actualslog,circpropM[,vinds[n]],lwd=3,col=nbdtpal[2])
  
  points(actualslog,tpropM3[,vinds[n]],lwd=3,pch=25,cex=1.5,col='black',bg='white')
  points(actualslog,tpropM2[,vinds[n]],lwd=3,pch=24,cex=1.5,col='grey',bg='grey')
  points(actualslog,tpropM[,vinds[n]],lwd=3,pch=23,cex=1.5,col=nbdtpal[3],bg=nbdtpal[3])
  points(actualslog,tsqpropM[,vinds[n]],lwd=3,pch=21,cex=1.5,col=nbdtpal[1],bg=nbdtpal[1])
  points(actualslog,circpropM[,vinds[n]],lwd=3,pch=22,cex=1.5,col=nbdtpal[2],bg=nbdtpal[2])
  
  text(1.1,0.95,paste('(',letters[n],')',sep=''),cex=2)
  
  if (n==1){legend(1.4,1,c('Two univariate, uncorrected','Two univariate, corrected',expression(T^2),expression('T'[circ]^2), 'Univariate amplitudes'),pch=c(24:25,21:23),cex=1.5,pt.bg=c('grey','white',nbdtpal[1:3]),col=c('grey','black',nbdtpal[1:3]),box.lwd=2)}
}
...
image(actualslog,dlist,tsqpropM,zlim=c(0,1),add=TRUE,col=kovesi.linear_blue_95_50_c20(256))
...
image(actualslog,dlist,circpropM,zlim=c(0,1),add=TRUE,col=kovesi.linear_blue_95_50_c20(256))
...
title(ttext,cex.main=2)
image(actualslog,dlist,tsqvscirc,zlim=c(0,1),add=TRUE,col=kovesi.linear_blue_95_50_c20(256))
cl <- contourLines(actualslog,dlist,tsqvscirc,levels=c(0.02,0.05,0.1,0.2,0.4))
...
pdf('figures/falsealarms.pdf', bg="transparent", height = 4, width = 15)
...
for (cond in 1:length(corvals)){
  for (simno in 1:nsims){
    ...
    if (output$pval < 0.05){csig[cond] <- csig[cond] + 1}
    output <- tsqh.test(simdata)
    if (output$pval < 0.05){hsig[cond] <- hsig[cond] + 1}
  }}
propc <- csig/nsims
...
knitr::include_graphics('figures/falsealarms.pdf')
...
Attempts to validate this by simulation (100,000 random data sets) found that for small sample sizes (_N_ < 10), there is a mismatch between the equation's predictions and the simulations (the black curve does not match the blue shading in Figure \@ref(fig:distcomparison)a). Instead, a closer approximation to the simulations is given by:
...
pdf('figures/distcomparison.pdf', bg="transparent", height = 5, width = 9)
...
The effectiveness of this approach to outlier exclusion can be assessed by simulation using the condition index test. Figure \@ref(fig:outlierplot) shows the proportion of significant condition index tests as a function of the Mahalanobis distance of a single outlier, for a range of sample sizes (curves). In all cases, the functions depart from the Type I error rate ($\alpha$ = 0.05; horizontal dashed line in Figure \@ref(fig:outlierplot)) when the outlier's Mahalanobis distance exceeds a value around 3. This seems a reasonable heuristic for outlier exclusion, and is the multivariate equivalent of excluding data points more than 3 standard deviations from the sample mean (note that many implementations of the Mahalanobis distance statistic, such as the core _mahalanobis_ function in _R_, or _mahal_ function in _Matlab_, return $D^2$, which can be converted to _D_ by taking the square root). Following this heuristic should reduce the likelihood that outliers will invalidate the assumptions of the $T^2_{circ}$ test.

```{r include=FALSE}

if (buildfigs==1){
  
pdf('figures/outlierplot.pdf', bg="transparent", height = 6, width = 7)
...
# this version avoids calling the condition index test to speed things up
# it means that the probability density function is not calculated on each iteration
...
for (s in 1:length(samplesizes)){
  ...
  for (d in 1:length(dlist)){
    for (n in 1:nsims){
      ...
      if(CI>criticalCI){nsig[s,d] <- nsig[s,d] + 1}
    }
  }
}
...
for (s in 1:length(samplesizes)){lines(dlist,propsig[s,],lwd=3,col=nbdtpal[s])}
...
knitr::include_graphics('figures/outlierplot.pdf')

```

A variant of the Mahalanobis distance (the pairwise Mahalanobis distance) can also be used to compute a multivariate measure of effect size, equivalent to Cohen's _d_ statistic [see e.g. @Giudice2009]. This is a valuable statistic to include when reporting the results of multivariate tests, and a function (_pairwisemahal_) to calculate it is available as part of the _FourierStats_ package.
...
where $N_k$ is the number of observations in group _k_, $\bar{x}_k$ is the bivariate sample mean of group _k_, $\bar{x}_{grand}$ is the bivariate mean of the entire sample (including all groups), $\Sigma$ denotes summation across groups, and $df_M$ is the degrees of freedom for the model. Note that by using the vector distances between points, this equation reduces multivariate data to a single scalar value, in a similar way to equations \@ref(eq:t2eq) and \@ref(eq:t2c). The model degrees of freedom for _k_ groups is $df_M = 2(k-1)$, with the factor of two scaling relative to standard univariate ANOVA reflecting the additional degrees of freedom afforded by having two dependent variables.
...
where $x_{i,k}$ is the $i$th data point from group _k_, and other terms are as described previously. The degrees of freedom for a balanced design are calculated as $df_R = 2((Nk)-k)$, where _N_ is the number of observations per group, and _k_ is the number of groups. A _p_-value can then be determined by comparing the F-ratio calculated from equation \@ref(eq:Fratio) to an F-distribution with $df_M$ and $df_R$ degrees of freedom.
...
pdf('figures/powerfig2.pdf', bg="transparent", height = 8, width = 12)
...
for (n in 1:nsims){
  ...
  if (output$pval<0.05){circmatrix[s,s2] <- circmatrix[s,s2] + 1}
...
title(ylab="Proportion significant", col.lab=rgb(0,0,0), line=2.2, cex.lab=1.5)
title(paste('d =',dlist[vinds[n]]),cex.main=2)
lines(actualslog,manovapropM[,vinds[n]],lwd=3,col=nbdtpal[1])
lines(actualslog,circpropM[,vinds[n]],lwd=3,col=nbdtpal[2])

points(actualslog,manovapropM[,vinds[n]],lwd=3,pch=15,cex=1.5,col=nbdtpal[1])
points(actualslog,circpropM[,vinds[n]],lwd=3,pch=16,cex=1.5,col=nbdtpal[2])
...
image(actualslog,dlist,manovapropM,zlim=c(0,1),add=TRUE,col=kovesi.linear_blue_95_50_c20(256))
text(1.1,3.8,'(d)',cex=2)
legend(3.4,3.8,c('0','0.5','1'),lwd=6,cex=1.5,col=kovesi.linear_blue_95_50_c20(3),box.lwd=2,bg='white',title='Power')
...
title(ttext,cex.main=2)
image(actualslog,dlist,manvscirc,zlim=c(0,1),add=TRUE,col=kovesi.linear_blue_95_50_c20(256))
cl <- contourLines(actualslog,dlist,manvscirc,levels=c(0.02,0.05,0.1,0.2,0.4))
for (n in 1:length(cl)){
  temp <- cl[n]
  lines(temp[[1]]$x,temp[[1]]$y,col='black',lwd=3)
}
...
knitr::include_graphics('figures/powerfig2.pdf')

```

Following a significant ${ANOVA}^2_{circ}$ test, one could calculate $T^2_{circ}$ statistics to make post-hoc pairwise comparisons between conditions, providing that appropriate multiple comparison correction is applied (e.g. Bonferroni correction). Univariate ANOVAs on the real and imaginary components are unlikely to be informative, as the relative magnitudes of the two measures depend on stimulus phase (which is arbitrary; see Figure \@ref(fig:fourierexplain)d). A repeated measures version of ${ANOVA}^2_{circ}$ can also be implemented following the same logic. The project repository contains a function (_anovacirc.test_ in _R_, _anovacirc_test_ in _Matlab_) to run both of these tests for one-way designs. In principle factorial versions might also be derived.
...
pdf('figures/flowchart.pdf', bg="transparent", height = 4, width = 6)
...
pdf('figures/mousedata.pdf', bg="transparent", height = 6, width = 10)
...
mahalaD <- pairwisemahal(fores, rep(1:2,each=6))
D <- mahalaD[1,2]
...
knitr::include_graphics('figures/mousedata.pdf')

```

Figure \@ref(fig:mousedata)a shows Fourier amplitude spectra at two frontal electrodes (marked black in the insets) for auditory stimulation (red) and optogenetic stimulation (blue), each at 40 Hz. There is a clear frequency-locked signal with approximately equal amplitude for each stimulation modality. Indeed, a paired univariate T-test on the amplitudes reveals no significant difference (_t_ = `r round(tresult$statistic,digits=2)`, _df_ = `r tresult$parameter`, _p_ = `r round(tresult$p.value,digits=2)`). However, inspection of the complex Fourier components for each condition suggests evidence of a phase difference between the two modalities (see Figure \@ref(fig:mousedata)b). The condition index test was non-significant for both conditions (sound: CI = `r round(cisound$CI,digits=2)`, _p_ = `r round(cisound$pval,digits=2)`; light: CI = `r round(cilight$CI,digits=2)`, _p_ = `r round(cilight$pval,digits=2)`), so a paired-samples $T^2_{circ}$ test was conducted. This revealed a significant difference between conditions ($T^2_{circ}$ = `r round(tsqcresult$tsqc,digits=2)`, $F_{(`r tsqcresult$df1`,`r tsqcresult$df2`)}$ = `r round(tsqcresult$Fratio,digits=2)`, _p_ = `r round(tsqcresult$pval,digits=3)`) with an effect size of _D_ = `r round(D,digits=2)`. This demonstrates that sound and (optogenetic) light entrain neural responses with different latencies (lags). The original study by Hwang et al. [-@Hwang2019] went on to explore interactions between these two signals.
...
pdf('figures/humanSSVEP.pdf', bg="transparent", height = 4, width = 14)
...
mahalaD <- pairwisemahal(fores, rep(1:2,each=89))
allD[cond-1] <- mahalaD[1,2]
...
<!-- Note that the appendix simulations below were added at the request of a reviewer, and then subsequently removed following the request of the same reviewer (and the editor). -->
<!-- # Appendix 1 - proof by stochastic simulation of asymptotic F-distributions -->

<!-- To confirm that distributions of F-statistics generated by the $ANOVA^2_{circ}$ test have the expected form, simulations were conducted for a range of study designs. A null bivariate data set was generated for _N_ participants and _k_ groups, using normally distributed random numbers with underlying condition means of 0 for both the real and imaginary components. The F-statistic was then calculated using equations \@ref(eq:Fratio), \@ref(eq:MSM) and \@ref(eq:MSR). This procedure was repeated one million times for twenty combinations of _N_ and _k_, and the distributions summarised using histograms. Figure \@ref(fig:fdistributions) shows the results of these simulations (blue shading), for comparison with analytically derived F-distributions of $df_M = 2(k-1)$ and $df_R = 2((Nk)-k)$ degrees of freedom (black lines, generated using the _df_ function in _R_). The correspondence between the stochastic and analytic distributions is excellent, confirming that the test produces the expected null distribution of test statistics. -->

<!-- ```{r include=FALSE} -->

<!-- if (buildfigs==1){ -->

<!--   pdf('figures/Fdistributions.pdf', bg="transparent", height = 5, width = 9) -->

<!--   nsims <- nsims*10  # increase to 1 million simulations for these -->
...
<!--       polygon(c(0,axvals,max(axvals))+(6*(s-1)), c(0,ayvals,0)+(4-s2), col=addalpha(nbdtpal[3],alpha=0.5),border=NA) -->
<!--       dfM <- 2*(nconds[s2]-1) -->
...
<!-- ```{r fdistributions, fig.cap = 'Null distributions of F-statistics generated by simulation (blue shading) and analytically (black lines) for different numbers of groups (k; rows) and sample sizes (N; columns).', fig.width=9, fig.height=5, fig.align="center", echo=FALSE} -->

<!-- knitr::include_graphics('figures/Fdistributions.pdf') -->
